        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 25) {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                    ...
                    MetricCard(title: "Monthly Revenue", value: String(format: "₹%.0f", earningsThisMonth), subtitle: "Current month earnings", icon: "indianrupeesign.circle.fill", color: .green)
                    MetricCard(title: "Cumulative Revenue", value: String(format: "₹%.0f", totalEarnings), subtitle: "Life-time earnings", icon: "banknote.fill", color: .blue)
                ...
                HStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        if monthlyEarningsData.isEmpty {
                            ...
                            .frame(height: 220)
                            .chartYAxis {
                                AxisMarks(format: .currency(code: "INR").precision(.fractionLength(0)))
                            }
                        }
                    }
                    .padding()
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                ...
                Button("Log Payment") {
                    guard selectedStudentId != UUID(), let amount = Double(amountString) else { return }
                ...
                .disabled(selectedStudentId == UUID() || Double(amountString) == nil)
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                ...
                Button("Add Student") {
                    guard !name.isEmpty, let rateValue = Double(rateValueString) else { return }
                    let student = Student(name: name, subject: subject, grade: grade, rateType: rateType, rateValue: rateValue, contactEmail: email, contactPhone: phone, scheduleNotes: scheduleNotes, notes: notes)
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                ...
                Button("Assign") {
                    guard selectedStudentId != UUID(), !title.isEmpty else { return }
                    let assignment = Assignment(studentId: selectedStudentId, title: title, description: description, dueDate: dueDate)
                    store.addAssignment(assignment)
                    dismiss()
                }
                .buttonStyle(.borderedProminent)
                .disabled(selectedStudentId == UUID() || title.isEmpty)
                .keyboardShortcut(.defaultAction)
            }
            .padding([.horizontal, .bottom])
        }
